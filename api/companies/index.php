...
try {
    switch ($method) {
        case 'GET':
            ...
            // Get companies for this client with assigned accountant info per company
            $stmt = $db->prepare("
                SELECT c.id, c.client_id, c.company_name, c.assigned_accountant_id,
                       c.created_at, c.updated_at,
                       u.email AS accountant_email, u.full_name AS accountant_full_name
                FROM companies c
                LEFT JOIN users u ON u.id = c.assigned_accountant_id
                WHERE c.client_id = ?
